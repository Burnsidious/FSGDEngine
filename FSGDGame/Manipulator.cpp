...
void Manipulator::OnMessage(EDGameCore::Message& message, const void *data)
{
	...
	if( message.messageId == MSG_DRAG )
	{
		if( manipulatedInstanceId == 0 )
		{
			...
			if( rayHit.collider != 0 )
			{
				...
				if( rayHit.collider->IntersectRay( start, normal, rayHit ) )	
					selectedInstanceId = rayHit.collider->GetGameObject()->GetInstanceId();
			}
...
void Manipulator::OnMessage(EDGameCore::Message& message)
{
	...
	if( message.messageId == MSG_RELEASE )
	{
		...
		if( manipulatedObject != 0 )
		{
			manipulatedObject->GetRigidBody()->Wake();
			dragEnabled = false;
		}
...
void Manipulator::FixedUpdate(void)
{
	if( manipulatedInstanceId != 0 && dragEnabled )
	{
		EDGameCore::GameObject* manipulatedObject = EDGameCore::GameObject::GetGameObjectInstance(manipulatedInstanceId);
		...
		manipulatedObject->GetRigidBody()->ApplyDisplacementAtPoint( delta, lastIntersection );
	}
}